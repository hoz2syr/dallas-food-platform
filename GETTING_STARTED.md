...
All backend APIs are protected by JWT authentication. You must obtain a JWT token from the Auth Service (`/api/auth/login` or `/api/auth/signup`) and include it in the `Authorization: Bearer <token>` header for all requests.

- **Auth Service**: `/signup`, `/login`, `/me` endpoints for user management and JWT issuance.
- **How to use:**
...
Some endpoints may be public for demo/testing. See each service's README for details.
